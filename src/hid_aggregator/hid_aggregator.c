...
static remapper_hid_source_state_t *find_or_allocate_source(
    ...
    remapper_hid_source_t source)
{
    remapper_hid_source_state_t *existing = find_source(aggregator, source);
    if (existing != NULL) {
        return existing;
    }
...
{
    if (aggregator == NULL || event == NULL ||
        !remapper_hid_source_is_valid(event->source)) {
        return false;
    }
    ...
    switch (event->type) {
        case REMAPPER_MOUSE_EVENT_BUTTON: {
            ...
            if (event->data.button.pressed) {
                ...
                const uint16_t mask = (uint16_t)(1u << (uint16_t)button);
                if ((slot->mouse_buttons & mask) == 0u) {
                    slot->mouse_buttons = (uint16_t)(slot->mouse_buttons | mask);
            ...
            if (slot != NULL) {
                const uint16_t mask = (uint16_t)(1u << (uint16_t)button);
                if ((slot->mouse_buttons & mask) != 0u) {
                    slot->mouse_buttons = (uint16_t)(slot->mouse_buttons & (uint16_t)~mask);
        ...
        case REMAPPER_MOUSE_EVENT_MOVE:
            aggregator->pending_dx = saturating_add_i32(
                aggregator->pending_dx, event->data.move.dx);
            aggregator->pending_dy = saturating_add_i32(
                aggregator->pending_dy, event->data.move.dy);
            return true;
...
bool remapper_hid_aggregator_release_source(
    ...
    remapper_hid_source_t source)
{
    if (aggregator == NULL || !remapper_hid_source_is_valid(source)) {
        return false;
    }
    ...
    for (size_t button = 0; button < REMAPPER_MOUSE_BUTTON_COUNT; ++button) {
        const uint16_t mask = (uint16_t)(1u << button);
        if ((slot->mouse_buttons & mask) != 0u &&
            aggregator->mouse_button_refs[button] > 0u) {
            --aggregator->mouse_button_refs[button];
        }
    }

    for (size_t key = 0; key < REMAPPER_HID_KEY_COUNT; ++key) {
        if (key_bitmap_get(slot->key_bitmap, (remapper_key_t)key) &&
            aggregator->key_refs[key] > 0u) {
            --aggregator->key_refs[key];
        }
    }

    for (size_t modifier = 0; modifier < REMAPPER_MOD_COUNT; ++modifier) {
        const uint8_t mask = (uint8_t)(1u << modifier);
        if ((slot->modifiers & mask) != 0u &&
            aggregator->modifier_refs[modifier] > 0u) {
            --aggregator->modifier_refs[modifier];
        }
    }
    ...
    remapper_hid_output_state_t *out_state)
{
    ...
    for (size_t button = 0; button < REMAPPER_MOUSE_BUTTON_COUNT; ++button) {
        if (aggregator->mouse_button_refs[button] > 0u) {
            out_state->mouse_buttons = (uint16_t)(
                out_state->mouse_buttons | (uint16_t)(1u << button));
        }
    }

    for (size_t key = 0; key < REMAPPER_HID_KEY_COUNT; ++key) {
        if (aggregator->key_refs[key] > 0u) {
            key_bitmap_set(out_state->key_bitmap, (remapper_key_t)key, true);
        }
    }

    for (size_t modifier = 0; modifier < REMAPPER_MOD_COUNT; ++modifier) {
        if (aggregator->modifier_refs[modifier] > 0u) {
            out_state->modifiers = (uint8_t)(
                out_state->modifiers | (uint8_t)(1u << modifier));
        }
    }
...
bool remapper_hid_output_key_is_down(const remapper_hid_output_state_t *state,
                                     remapper_key_t key)
{

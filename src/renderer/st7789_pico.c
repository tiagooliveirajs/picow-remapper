...
static bool valid_rect(uint16_t x,uint16_t y,uint16_t width,uint16_t height){return width>0u&&height>0u&&x<REMAPPER_RENDERER_WIDTH&&y<REMAPPER_RENDERER_HEIGHT&&(uint32_t)x+width<=REMAPPER_RENDERER_WIDTH&&(uint32_t)y+height<=REMAPPER_RENDERER_HEIGHT;}
static bool pico_fill_rect(void *context,uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint16_t rgb565){(void)context;if(!valid_rect(x,y,width,height))return false;static uint8_t row[REMAPPER_RENDERER_WIDTH*2u];for(uint16_t column=0u;column<width;++column){row[(size_t)column*2u]=(uint8_t)(rgb565>>8);row[(size_t)column*2u+1u]=(uint8_t)rgb565;}lcd_window(x,y,width,height);gpio_put(LCD_PIN_DC,true);lcd_select(true);for(uint16_t line=0u;line<height;++line)(void)spi_write_blocking(LCD_SPI,row,(size_t)width*2u);lcd_select(false);return true;}
...
void remapper_st7789_pico_set_backlight(bool enabled){gpio_put(LCD_PIN_BL,enabled);}
...
bool remapper_st7789_pico_init(remapper_display_hal_t *display){if(display==NULL)return false;(void)spi_init(LCD_SPI,LCD_SPI_BAUD_HZ);gpio_set_function(LCD_PIN_SCK,GPIO_FUNC_SPI);gpio_set_function(LCD_PIN_MOSI,GPIO_FUNC_SPI);const uint outputs[]={LCD_PIN_DC,LCD_PIN_CS,LCD_PIN_RST,LCD_PIN_BL};for(size_t i=0u;i<sizeof(outputs)/sizeof(outputs[0]);++i){gpio_init(outputs[i]);gpio_set_dir(outputs[i],GPIO_OUT);}gpio_put(LCD_PIN_CS,true);gpio_put(LCD_PIN_DC,false);gpio_put(LCD_PIN_RST,true);remapper_st7789_pico_set_backlight(false);sleep_ms(100u);gpio_put(LCD_PIN_RST,false);sleep_ms(100u);gpio_put(LCD_PIN_RST,true);sleep_ms(100u);configure_panel();display->context=NULL;display->fill_rect=pico_fill_rect;display->write_rgb565=pico_write_rgb565;return true;}

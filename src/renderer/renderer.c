...
{
    if (frame==NULL || text==NULL || row>=REMAPPER_RENDERER_TEXT_ROWS || column>=REMAPPER_RENDERER_TEXT_COLS) return false;
    size_t index=0u;
...
{
    if (frame==NULL || row>=REMAPPER_RENDERER_TEXT_ROWS || column>=REMAPPER_RENDERER_TEXT_COLS || (size_t)column+length>REMAPPER_RENDERER_TEXT_COLS) return false;
    for (size_t index=0u; index<length; ++index) frame->cells[row][(size_t)column+index].tone=tone;
    return true;
}
...
static bool draw_cell(const remapper_display_hal_t *display, const remapper_ui_frame_t *frame, uint8_t row, uint8_t column)
{
    ...
    const uint16_t y=(uint16_t)(REMAPPER_RENDERER_TEXT_Y+(uint16_t)row*REMAPPER_RENDERER_LINE_ADVANCE);
    return display->write_rgb565(display->context,x,y,REMAPPER_RENDERER_GLYPH_WIDTH,REMAPPER_RENDERER_GLYPH_HEIGHT,pixels);
}
...
bool remapper_renderer_render(const remapper_display_hal_t *display, const remapper_ui_frame_t *frame)
{
    ...
    if (frame->learn_background) {
        if (!display->fill_rect(display->context,0u,0u,REMAPPER_RENDERER_WIDTH,REMAPPER_RENDERER_HEIGHT,COLOR_DARK_MAGENTA)) return false;
    } else {
        const uint16_t boundary=remapper_renderer_separator_boundary_y(frame);
        if (boundary>0u && !display->fill_rect(display->context,0u,0u,REMAPPER_RENDERER_WIDTH,boundary,COLOR_BLACK)) return false;
        if (boundary<REMAPPER_RENDERER_HEIGHT && !display->fill_rect(display->context,0u,boundary,REMAPPER_RENDERER_WIDTH,(uint16_t)(REMAPPER_RENDERER_HEIGHT-boundary),COLOR_DARK_MAGENTA)) return false;
    }

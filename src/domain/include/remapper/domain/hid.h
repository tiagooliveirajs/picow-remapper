...
typedef enum {
    ...
    REMAPPER_HID_SOURCE_MOUSE = 1,
    REMAPPER_HID_SOURCE_KEYBOARD = 2,
    REMAPPER_HID_SOURCE_COMPOSITE = 3,
    REMAPPER_HID_SOURCE_SYNTHETIC_REMAP = 4,
...
static inline remapper_hid_source_t remapper_hid_source_make(
    ...
    uint16_t instance)
{
    remapper_hid_source_t source = {(uint16_t)kind, instance};
    return source;
}
...
/*
 * Canonical keyboard keys use the USB HID Keyboard/Keypad Usage ID namespace.
 * This is a logical key identity only; transport report IDs, bit offsets and
 * remote descriptor layouts are deliberately absent from the domain model.
 */
